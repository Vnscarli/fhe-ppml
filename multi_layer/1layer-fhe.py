...
def calculate_cleartext_expected(data, weights_matrix):
    ...
    for weights in weights_matrix:
        # Dot product in plain text
        dot_product = sum(d * w for d, w in zip(data, weights))
        
        # Sigmoid approximation (-0.004*x^3 + 0.197*x + 0.5)
...
def main():
    ...
    expected_results = calculate_cleartext_expected(data, weights_matrix)

    # Compare to check noise
    for i in range(num_neurons):
        valid_index = i * len(data)
        val_fhe = res_array[valid_index]
        
        dot_clear, val_clear = expected_results[i]
        
        error = abs(val_fhe - val_clear)
        
        print(f"Neuron {i}:")
        print(f"  Raw Dot Product:             {dot_clear:.4f}")
        print(f"  Expected Output (Cleartext): {val_clear:.8f}")
        print(f"  FHE Output (Ciphertext):     {val_fhe:.8f}")
        print(f"  Absolute Error (Noise):      {error:.8f}\n")
